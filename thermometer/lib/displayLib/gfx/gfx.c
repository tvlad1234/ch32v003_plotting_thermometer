...
void gfx_draw_bmp_rgb565_internal(gfx_inst *gfx, int16_t x, int16_t y, const uint16_t bitmap[], int16_t w, int16_t h)
{
	uint16_t *ptr = bitmap;
	for (int i = 0; i < h; i++) // for each column
	{
		for (int j = 0; j < w; j++) // draw a row
			gfx_draw_pixel(gfx, x+j, y, *(ptr++));
		y++;
	}
}

void gfx_draw_bmp_rgb565(gfx_inst *gfx, int16_t x, int16_t y, const uint16_t bitmap[], int16_t w, int16_t h)
{
	if(gfx->rgb565_bmp_fun)
		(*(gfx->rgb565_bmp_fun))(gfx->disp_ptr, x, y, bitmap, w, h);
	else gfx_draw_bmp_rgb565_internal(gfx, x, y, bitmap, w, h);
}