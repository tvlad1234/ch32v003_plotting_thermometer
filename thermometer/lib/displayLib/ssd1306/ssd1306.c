...
{
	oled->i2cdev->tx_mem( NULL, oled->i2c_addr, SSD1306_ADDR_CMD, &c, 1 );
}

void ssd1306_command_list( ssd1306_oled *oled, const uint8_t *c, uint8_t n )
{
	oled->i2cdev->tx_mem( NULL, oled->i2c_addr, SSD1306_ADDR_CMD, c, n );
}

void ssd1306_framebuf_transmit( ssd1306_oled *oled )
{
	oled->i2cdev->tx_mem( NULL, oled->i2c_addr, SSD1306_ADDR_FB, oled->frameBuffer, 1024 );
}

void oled_init( ssd1306_oled *oled, i2cdev_t *i2c, uint8_t addr, uint8_t vccstate, uint16_t w, uint16_t h )
{
	oled->width = w;
	oled->height = h;
	oled->inversion = 0;
	...
	ssd1306_oled *oled, int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color )
{
	...
	for ( int16_t j = 0; j < h; j++, y++ )
	{
		for ( int16_t i = 0; i < w; i++ )
		{
			...
			if ( byte & 0x80 ) oled_draw_pixel( oled, x + i, y, color );
		}
	}
}
...
void oled_draw_pixel( ssd1306_oled *oled, int16_t x, int16_t y, uint16_t color )
{
	if ( ( x >= 0 ) && ( x < oled->width ) && ( y >= 0 ) && ( y < oled->height ) )
	{
		...
		switch ( color )
		{
			case SSD1306_WHITE: oled->frameBuffer[x + ( y / 8 ) * oled->width] |= ( 1 << ( y & 7 ) ); break;
			case SSD1306_BLACK: oled->frameBuffer[x + ( y / 8 ) * oled->width] &= ~( 1 << ( y & 7 ) ); break;
			case SSD1306_INVERSE: oled->frameBuffer[x + ( y / 8 ) * oled->width] ^= ( 1 << ( y & 7 ) ); break;
		}
	}
}
